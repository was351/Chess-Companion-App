...
Anyone implementing must mirror these todos in the agent session and update YAML statuses (`pending` → `in_progress` → `completed`) as work completes. No installs/tests/code edits until todos exist for that slice.
...
| `engine:job:{job_id}` | HASH | `status`, `fen`, `payload_json`, `result_json`, `attempts`, `created_at`, `updated_at`, `claimed_at` (set when entering `processing` for reclaim), `error`, `cancel_requested` |
...
**Stored in Redis after enqueue (worker always sees this):**
...
1. Compute **`dedupe_key`** from canonical inputs (see **Reliability rules** below).
...
- **Env:** `REDIS_ENGINE_URL`, `STOCKFISH_PATH` only. **No Supabase in worker** if API normalizes archived positions to `fen` before enqueue (see Job payload).
...
- **Engine lifecycle:** one **long-lived** `SimpleEngine.popen_uci(STOCKFISH_PATH)` per process; only the main thread touches UCI in v1. **`threading.Lock`** around UCI is optional future-proofing if you ever add concurrent engine use; not required if only the main thread runs Stockfish. Alternative: **new subprocess per job** (slower, stronger isolation).
...
- Second **thread** or periodic timer: **`LRANGE engine:queue:processing`**, then for each `job_id` **`HGET`** `claimed_at` / `status` (see **Reliability rules**); **`LREM`** + **`LPUSH` ready** or DLQ. **Never** call Stockfish from this thread. Same binary, no extra service for v1.
...
- **Scale:** `docker compose up --scale engine-worker=3` — each replica runs the same **`BRPOPLPUSH`** claim loop; one worker wins each atomic move from `ready` → `processing`; **`LREM`** on done/fail clears `processing`.
...
- **`failed`:** `error` string on hash; `result_json` may carry `{ "status": "failed", "error": "..." }`.

**SSE `data:`** lines: JSON with at least **`job_id`**, **`fen`**, **`status`**, and latest **`lines`** / scores when present.
...
## Reliability rules (unchanged intent, build-aligned)
...
- **Reclaimer:** **`LRANGE engine:queue:processing 0 -1`** (or iterate your processing list) and, for each **`job_id`**, **`HGET`** `claimed_at` / `status`; if **`now - claimed_at` > visibility timeout** and `status` still `running`, **`LREM engine:queue:processing 1 {job_id}`**, then **`LPUSH engine:queue:ready {job_id}`** or DLQ if `attempts >= max_attempts`. Requires **`claimed_at`** set at claim time.
...
- Add `REDIS_ENGINE_URL` (or db `/1`) config next to `REDIS_URL` in [`Board-Backend`](../Board-Backend/).
...
- Implement `GET .../events` mirroring [`Board-Backend/game/routes.py`](../Board-Backend/game/routes.py) SSE pattern (keepalive, disconnect cleanup).
...
- New package: **`Board-Backend/engine/`** — `queue.py`, `jobs.py`, `schemas.py`, `routes.py`.
- Register router in [`Board-Backend/api.py`](../Board-Backend/api.py) at prefix `/engine` (or `/api/engine` — match existing style).
- Worker: **`Board-Backend/engine_worker/`** or sibling **`Board-EngineWorker/`** with its own `Dockerfile` + **`CMD`** e.g. **`["python", "-m", "engine_worker"]`** (module runs the sync `BRPOPLPUSH` loop until SIGTERM).
- Compose: extend [`docker/stack.yml`](../docker/stack.yml) (and local [`Board-Backend/docker-compose.yml`](../Board-Backend/docker-compose.yml)) with `engine-worker` service, `depends_on: redis`, env `REDIS_ENGINE_URL`, `STOCKFISH_PATH`.

---

## After this plan (implementation order)
...
_Last updated: 2026-04-10 — plan complete for v1 implementation._
